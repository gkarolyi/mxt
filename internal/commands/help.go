...
func HelpCommand(version string) {
	...
	fmt.Printf("    %sconfig migrate%s                    Convert legacy key=value config to TOML\n", ui.Cyan, ui.Reset)
	fmt.Println()
	...
	fmt.Println("    Global:  ~/.mxt/config (TOML)")
	fmt.Println("             (worktree_dir, terminal, copy_files, pre_session_cmd, tmux_layout)")
	fmt.Println("    Project: .mxt in repo root      (TOML overrides global settings)")
	fmt.Println("    Legacy:  mxt config migrate     (convert key=value configs)")
	...
	fmt.Println("                        \"\"\"")
	fmt.Println()
	fmt.Println("                        Single-line format: tmux_layout = \"dev:hx|lazygit,server:bin/server,agent:\"")
	fmt.Println()
	fmt.Println("                        Syntax:")
