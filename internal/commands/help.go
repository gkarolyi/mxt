...
// HelpCommand prints the help output matching muxtree.
func HelpCommand(version string) {
	...
	fmt.Printf("%sUSAGE%s\n", ui.Bold, ui.Reset)
	fmt.Println("    muxtree <command> [options]")
	fmt.Println()

	fmt.Printf("%sCOMMANDS%s\n", ui.Bold, ui.Reset)
	fmt.Printf("    %sinit%s                              Set up global config (~/.muxtree/config)\n", ui.Cyan, ui.Reset)
	fmt.Println("        --local                       Create project config (.muxtree in repo root)")
	...
	fmt.Println("    muxtree init                          # Global setup")
	fmt.Println("    muxtree init --local                  # Project-specific copy_files")
	fmt.Println("    muxtree new feature-auth              # New worktree from main")
	fmt.Println("    muxtree new fix-bug --from develop    # New worktree from develop")
	fmt.Println("    muxtree new feature-ai --run claude   # Auto-launch claude code")
	fmt.Println("    muxtree new fix-bug --bg              # Create without opening terminals")
	fmt.Println("    muxtree list                          # Show all worktrees + status")
	fmt.Println("    muxtree sessions close feature-auth   # Kill tmux sessions")
	fmt.Println("    muxtree sessions relaunch fix-bug     # Restart sessions")
	fmt.Println("    muxtree delete feature-auth           # Remove worktree + branch")
	fmt.Println()
	fmt.Printf("%sCONFIG%s\n", ui.Bold, ui.Reset)
	fmt.Println("    Global:  ~/.muxtree/config")
	fmt.Println("             (worktree_dir, terminal, copy_files, pre_session_cmd, tmux_layout)")
	fmt.Println("    Project: .muxtree in repo root      (overrides global settings)")
	fmt.Println("    Env:     MUXTREE_CONFIG_DIR=/path    (override global config dir)")
	fmt.Println()
